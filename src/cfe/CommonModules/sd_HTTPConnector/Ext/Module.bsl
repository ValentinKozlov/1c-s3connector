...
Функция ОбъединитьЗаголовки(ГлавныйИсточник, ДополнительныйИсточник)
	...
	Заголовки = Новый Соответствие;
	Для Каждого ЗаголовокЗапроса Из ГлавныйИсточник Цикл
		Заголовки.Вставить(ЗаголовокЗапроса.Ключ, ЗаголовокЗапроса.Значение);
	КонецЦикла;
	Для Каждого Заголовок Из ДополнительныйИсточник Цикл
		Если Заголовки.Получить(ЗаголовокЗапроса.Ключ) = Неопределено Тогда
			Заголовки.Вставить(ЗаголовокЗапроса.Ключ, ЗаголовокЗапроса.Значение);
		КонецЕсли;
	КонецЦикла;
...
Функция ОтправитьHTTPЗапрос(Сессия, ПодготовленныйЗапрос, Настройки)
	...
	//++Frog 16.03.2022
	//++Ватагина,2021-09-14, доп логирование 	
	Если (Ответ.КодСостояния <> 200) и (Ответ.КодСостояния <> 204) Тогда
		ТекстСообщ = Ответ.ПолучитьТелоКакСтроку();
		
		Если ТекстСообщ <> "" Тогда
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщ);			
		КонецЕсли;
	КонецЕсли;
	//--Ватагина,2021-09-14, доп логирование 
	//--Frog 16.03.2022
